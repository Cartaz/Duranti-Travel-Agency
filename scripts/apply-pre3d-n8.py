...
# Fix the runtime-config import so Node's native TypeScript test runner resolves it too.
replace_once(
    'src/data/external/nominatim-place-discovery.ts',
    "import { resolveNominatimEndpoint } from './runtime-config'",
    "import { resolveNominatimEndpoint } from './runtime-config.ts'",
)

# IndexedDB v2 is index-only: preserve v1, add reverse-reference indexes in v2.
replace_once('src/data/db/dtagency-db.ts', 'export const DB_VERSION = 1', 'export const DB_VERSION = 2')
replace_once('src/data/db/dtagency-db.ts', 'const STORES = {', 'const STORES_V1 = {')
replace_once(
    'src/data/db/dtagency-db.ts',
    '} as const\n\nexport class DTAgencyDatabase',
...
# Place deletion must use reverse-reference indexes instead of table scans.
replace_once(
    'src/data/repositories/place-repository.ts',
