...
const itineraryTypes = new Set<EditableItineraryType>([
  'transport',
  'activity',
  'meal',
  'reservation',
...
async function normalizeManualDraft(tripId: string, dayId: string, input: ItineraryDraft): Promise<ItineraryDraft> {
  ...
  if (startsAt && startsAt.slice(0, 10) !== day.date) {
    throw new Error('L’inizio della tappa deve cadere nella giornata corrente.')
  }
  ...
  if (endsAt && trip.endDate && endsAt.slice(0, 10) > trip.endDate) {
    throw new Error('La fine della tappa non può superare la data di ritorno del viaggio.')
  }
  ...
  return {
    ...
    placeId,
    bookingReference: validateOptionalText(input.bookingReference, 'Riferimento', 200),
...
export function itineraryToDraft(itinerary: Itinerary): ItineraryDraft {
  return {
    ...
    bookingReference: itinerary.bookingReference,
    notes: itinerary.notes,
  }
}

export async function listItineraryPlaces(): Promise<Place[]> {
...
export async function listDayItineraryItems(tripId: string, dayId: string): Promise<DayItineraryItem[]> {
  ...
  const legacyItems: DayItineraryItem[] = reservations.flatMap((reservation) => {
...
export async function saveManualItineraryItem(
  tripId: string,
  dayId: string,
  ...
  if (current && (current.reservationId || current.blockId)) {
    throw new Error('Le tappe derivate da prenotazioni si modificano dal relativo blocco del planner.')
  }
  if (current && (current.tripId !== tripId || current.dayId !== dayId)) {
  ...
  if (!current) {
    const siblings = (await itineraryRepository.list()).filter((item) => item.tripId === tripId && item.dayId === dayId)
...
export async function deleteManualItineraryItem(tripId: string, dayId: string, itineraryId: string): Promise<void> {
  ...
  if (itinerary.reservationId || itinerary.blockId) {
    throw new Error('Le tappe derivate da prenotazioni si eliminano dal relativo blocco del planner.')
  }
  await itineraryRepository.softDelete(itineraryId)
}
...
export async function reconcileDayReservationItineraries(tripId: string, dayId: string): Promise<number> {
  ...
  for (const block of blocks) {
    ...
    await reservationBlockRepository.saveReservationForBlock(block.id, tripId, dayId, reservation)
    reconciled += 1
  }
