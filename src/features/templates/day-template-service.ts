...
import { createDayApplication, type DayDraft } from '../../application/days/day-application'
import { requireTripDay } from '../../application/shared/trip-day-context'
...
const dayApplication = createDayApplication({
  days: dayRepository,
  trips: { getTrip: (tripId) => tripRepository.get(tripId) },
...
function reusableChecklistContent(content: Record<string, unknown>, regenerateIds: boolean): Record<string, unknown> {
  ...
  const items = content.items.flatMap((item, index) => {
    ...
    if (!text) return []
    return [{ id: regenerateIds ? crypto.randomUUID() : `template-item-${index + 1}`, text, checked: false }]
  }).slice(0, 100)
  return { items }
}

function reusableBlockContent(type: BlockType, content: Record<string, unknown>, regenerateChecklistIds: boolean): Record<string, unknown> {
  switch (type) {
    case 'text': return { text: cleanReusableText(content.text, 10_000) }
    case 'heading': return { text: cleanReusableText(content.text, 200) }
    case 'checklist': return reusableChecklistContent(content, regenerateChecklistIds)
    case 'divider':
    case 'place':
    case 'transport':
    case 'accommodation':
...
export async function createPersonalDayTemplate(tripId: string, dayId: string, input: PersonalDayTemplateDraft): Promise<Template> {
  const { trip, day } = await requireTripDay({ trips: tripRepository, days: dayRepository }, tripId, dayId)
  ...
  const definition: Template['definition'] = {
    blocks: blocks.map((block, index) => ({ type: block.type, position: index + 1, content: reusableBlockContent(block.type, block.content, false) })),
  }
  const now = new Date().toISOString()
  const template: Template = {
...
export async function createTripDayFromTemplate(tripId: string, draft: DayDraft, templateId: string): Promise<Day> {
  await ensureBuiltInDayTemplates()
  ...
  const day = await dayApplication.createTripDay(tripId, draft)
