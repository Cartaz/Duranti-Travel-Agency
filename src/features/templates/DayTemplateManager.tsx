...
import { MAX_DAY_TEMPLATE_NAME_LENGTH } from './day-template-service'
import {
  deletePersonalDayTemplate,
  renamePersonalDayTemplate,
} from './personal-day-template-service'
import './day-template-manager.css'

export default function DayTemplateManager({
  templates,
  disabled = false,
  onChanged,
}: {
  ...
  const saveRename = async (template: Template): Promise<void> => {
    ...
    setStatus('')
    try {
      const updated = await renamePersonalDayTemplate(template.id, name)
      setEditingId(undefined)
      setName('')
      await onChanged()
      setStatus(`Modello rinominato in “${updated.name}”.`)
    } catch (cause) {
  ...
  const remove = async (template: Template): Promise<void> => {
    ...
    setStatus('')
    try {
      await deletePersonalDayTemplate(template.id)
      setDeleteId(undefined)
      if (editingId === template.id) cancelRename()
      await onChanged()
      setStatus(`Modello “${template.name}” eliminato.`)
    } catch (cause) {
  ...
  return (
    <details className="day-template-manager">
      <summary>
        <span>
          <strong>Gestisci modelli personali</strong>
          <small>{templates.length} {templates.length === 1 ? 'modello salvato' : 'modelli salvati'}</small>
        </span>
      </summary>
          ...
          {templates.map((template) => {
            ...
            return (
              <article className="day-template-manager-item" key={template.id}>
                      ...
                      <input
                        ...
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            ...
                            void saveRename(template)
                          }
                          if (event.key === 'Escape') cancelRename()
                        }}
                      />
                      <small>{name.length}/{MAX_DAY_TEMPLATE_NAME_LENGTH}</small>
                    </label>
                  ) : (
                    <>
                ...
                {!deleting && (
                  <div className="day-template-manager-actions">
                    {editing ? (
                      <>
                        <button type="button" disabled={disabled || busy} onClick={cancelRename}>Annulla</button>
                        <button type="button" disabled={disabled || busy || !name.trim()} onClick={() => void saveRename(template)}>
                          {busy ? 'Salvataggio…' : 'Salva nome'}
                        </button>
                      </>
                    ) : (
                      <>
                        <button type="button" disabled={disabled || Boolean(busyId)} onClick={() => startRename(template)}>Rinomina</button>
                        <button className="day-template-manager-delete" type="button" disabled={disabled || Boolean(busyId)} onClick={() => {
                          setEditingId(undefined)
                ...
                {deleting && (
                  <InlineConfirm
                    title="Eliminare questo modello personale?"
                    message={`“${template.name}” non comparirà più tra le strutture disponibili. Le giornate già create con questo modello non verranno modificate.`}
