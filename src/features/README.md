...
- `expenses/` — exact minor-unit expense records linked atomically to planner blocks, payer assignment restricted to trip participants, derived trip summaries separated by currency with per-day/category/payer breakdowns, and budget progress calculated only against expenses in the trip budget currency.
...
- `expenses/` — optional explicit FX workflows.
